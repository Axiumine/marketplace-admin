...
2. **A branch that cannot be reached.** Delete it rather than testing it. Two of them were removed
   while this suite was written: a second `?? []` on a list the empty check had already unwrapped, and
   a `{session?.email ?? ''}` whose fallback rendered a bordered empty strip in a state that is real —
   `useLogout` clears the session before it navigates.
...
## Two determinism traps this suite already hit
