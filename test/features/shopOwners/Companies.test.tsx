...
/** A second company of the same shopOwner, for the tests that need two cards on the page. */
const companyTwo = {
	...company,
...
/**
 * The page's two queries, of which the companies are one.
 */
...
const OK_ADD = { CompanyAdd: { data: { companyAdd: true } } }
const OK_DEL = { CompanyDel: { data: { companyDel: true } } }

/**
 * ⚠️ This is the only caller of `shopOwnerCompanies`, and it sits below the personalData on one detail
 * page. Both take the same `idShopOwner` and render the same kind of box, so a copy of the neighbouring
 * section left pointed at the wrong query renders a plausible-looking page with one shopOwner's details
 * shown twice and no company anywhere.
 */
describe('Companies', () => {
	it('waits before claiming there are no companies', async () => {
		stubGraphQL({ ...personalData, ShopOwnerCompanies: { pending: true } })
	...
	it('renders every company of the shopOwner', async () => {
...
describe('Companies — editing', () => {
	...
	it('refetches the companies after a rename', async () => {
		const stub = stubGraphQL({
			...personalData,
			ShopOwnerCompanies: { data: { shopOwnerCompanies: [company] } },
			...OK
		})
		await renderRoute(DETAIL)
...
describe('Companies — new company', () => {
	...
	// stays there when it fails — an shopOwner with no company is exactly who needs it.
	it('offers the plus while the companies are still loading', async () => {
		stubNetwork({ ...personalData, ShopOwnerCompanies: { pending: true } })
		await renderRoute(DETAIL)
	...
	// The whole point of the card. `idShopOwner` where the update sends `_id`, the same input object
