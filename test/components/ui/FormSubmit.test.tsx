...
describe('FormSubmit', () => {
	it('submits the form it ends', () => {
		render(<FormSubmit>Crea imprenditore</FormSubmit>)
		expect(screen.getByRole('button', { name: 'Crea imprenditore' })).toHaveAttribute('type', 'submit')
	})
	...
	it('sits on a full-width row that pins it to the right edge', () => {
		const { container } = render(<FormSubmit>Crea imprenditore</FormSubmit>)

		const row = container.firstChild
		expect(row).toHaveClass('col-span-full', 'flex', 'justify-end')
		expect(screen.getByRole('button', { name: 'Crea imprenditore' })).toHaveClass('px-[30px]')
	})
	...
	it('shows the spinner and blocks a second submit while the form is in flight', () => {
		render(<FormSubmit loading>Crea imprenditore</FormSubmit>)

		expect(screen.getByRole('button', { name: /Crea imprenditore/ })).toBeDisabled()
