 ...
 * admin's rows: the reads that matter here take no variables at all, which makes the keys identical
 * across the two sessions. A full load rebuilds every module at once — the cache, the token store, the
 * session store, and anything a later feature parks at module scope — which is what makes this correct
 * by construction rather than a list of stores somebody has to remember to extend.
 *
