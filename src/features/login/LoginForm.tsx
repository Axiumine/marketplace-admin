 ...
 * session — one day unchecked, thirty checked — which every later refresh is measured against. The cookie's
 * own lifetime does not change with it, so pinning the box here would silently decide how long an admin
 * stays signed in without changing a single thing the browser can see.
 *
 ...
 * IP. With no site key configured the widget renders nothing and the form submits a `null` token,
 * which is the correct request for a deployment whose server holds no secret key either.
 */
