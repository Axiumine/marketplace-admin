 ...
 * arrive, and a map centred on Rome for it would be a claim rather than a gap. A punto vendita always
 * has one — the field is required on its collection — while an imprenditore may not, since the point
 ...
 *
 * The missing `position` of an imprenditore stored before the point existed is answered here too,
 ...
 */
export const puntoDiMappa = (coordinates: number[] | undefined): { lat: number; lon: number } | null => {
 ...
 */
export const coordinateTesto = (coordinates: number[]): { longitudine: string; latitudine: string } => ({
	longitudine: String(coordinates[0] ?? ''),
	latitudine: String(coordinates[1] ?? '')
})
 ...
 *
 * The provincia is upper-cased here because the schemas upper-case their own: a record stored with a
 * lower-case sigla would otherwise seed a box that disagrees with the form before anything was typed.
 */
export const indirizzoComposto = (indirizzo: { indirizzo: string; cap: string; comune: string; provincia: string }): string =>
	formatIndirizzo({ ...indirizzo, provincia: indirizzo.provincia.toUpperCase() })
 ...
 */
type ErroreCampo = { message?: string | undefined } | undefined

/**
 * The seven keys every address form here shares, whatever else its own values type carries.
 *
 * Exported for `erroriForm.ts` alone, which walks a `FieldErrors` — react-hook-form's *untyped* error
 ...
 */
export type ErroriIndirizzo = {
	indirizzo?: ErroreCampo
	cap?: ErroreCampo
	comune?: ErroreCampo
	provincia?: ErroreCampo
	latitudine?: ErroreCampo
	longitudine?: ErroreCampo
	indirizzoCompleto?: ErroreCampo
}
 ...
 * with no CAP is the case that happens — would block the save with nothing on screen saying why.
 *
 ...
 * broken: "seleziona l'indirizzo dall'elenco" under an address that *was* selected, and whose CAP is
 * the problem, sends the operator back to the list to pick it again for nothing.
 *
 * Shared by all three cards that edit an address — anagrafica, azienda, punto vendita. It was written
 ...
 */
export const erroreIndirizzo = (errors: ErroriIndirizzo): string | undefined =>
	[
		errors.indirizzo,
		errors.cap,
		errors.comune,
		...
		errors.latitudine,
		errors.longitudine,
		errors.indirizzoCompleto
	].find((errore) => errore !== undefined)?.message
