 ...
 * `/admin-authenticated-resource`, …) that nginx proxies to the seven services. That is not a
...
export default defineConfig(({ mode }) => {
	...
	return {
		...
		server: {
			host: '127.0.0.1',
			port: Number(env.PORT ?? 3043),
			proxy: {
				'/public-authorization': 'http://127.0.0.1:4028',
				'/admin-authenticated-authorization': 'http://127.0.0.1:4025',
				'/admin-authenticated-resource': 'http://127.0.0.1:4024',
